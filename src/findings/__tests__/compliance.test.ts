...
describe('complianceReport', () => {
  it('all frameworks', () => { expect(complianceReport([makeFinding({cweId:'CWE-89',title:'SQLi'})]).reports).toHaveLength(3); });
  it('totalFindings', () => { expect(complianceReport([makeFinding({cweId:'CWE-89'}),makeFinding({cweId:'CWE-79'})]).totalFindings).toBe(2); });
  it('empty coverage', () => { expect(complianceReport([]).coveragePercent).toBe(0); });
  it('totalCovered', () => { expect(complianceReport([makeFinding({cweId:'CWE-89',title:'SQLi'})]).totalCovered).toBeGreaterThan(0); });
});
