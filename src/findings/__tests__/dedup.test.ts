...
describe('deduplicateWithCounts', () => {
  it('empty', () => { expect(deduplicateWithCounts([])).toEqual([]); });
  it('single', () => { const r = deduplicateWithCounts([makeFinding({title:'U',file:'a.ts',line:1})], 'exact'); expect(r).toHaveLength(1); expect(r[0].duplicateCount).toBe(1); });
  it('group size', () => { const r = deduplicateWithCounts([makeFinding({title:'SQL',file:'a.ts',line:10,confidence:0.9,scanner:'semgrep'}),makeFinding({title:'SQL',file:'a.ts',line:10,confidence:0.7,scanner:'trivy'}),makeFinding({title:'SQL',file:'a.ts',line:10,confidence:0.5,scanner:'eslint'})], 'exact'); expect(r).toHaveLength(1); expect(r[0].duplicateCount).toBe(3); });
  it('merges sources', () => { const r = deduplicateWithCounts([makeFinding({title:'SQL',file:'a.ts',line:10,confidence:0.9,scanner:'semgrep'}),makeFinding({title:'SQL',file:'a.ts',line:10,confidence:0.7,scanner:'trivy'})], 'exact'); expect(r[0].mergedSources).toContain('semgrep'); expect(r[0].mergedSources).toContain('trivy'); });
  it('sorts by severity', () => { const r = deduplicateWithCounts([makeFinding({title:'L',severity:'low',file:'a.ts',line:1}),makeFinding({title:'C',severity:'critical',file:'b.ts',line:2})], 'exact'); expect(r[0].severity).toBe('critical'); });
  it('fuzzy default', () => { expect(deduplicateWithCounts([makeFinding({title:'SQL injection in query builder',file:'a.ts',line:10,confidence:0.9}),makeFinding({title:'SQL injection in query builders',file:'b.ts',line:20,confidence:0.7})])).toHaveLength(1); });
  it('source fallback', () => { expect(deduplicateWithCounts([makeFinding({title:'F',file:'a.ts',line:1,scanner:undefined,source:'llm'})], 'exact')[0].mergedSources).toContain('llm'); });
});
