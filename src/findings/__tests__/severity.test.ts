...
describe('scoreSeverity', () => {
  it('base score', () => { const s = scoreSeverity('high', baseContext()); expect(s.value).toBe(70); expect(s.level).toBe('high'); expect(s.reasoning).toHaveLength(1); });
  it('base values', () => { const c = baseContext(); expect(scoreSeverity('critical',c).value).toBe(90); expect(scoreSeverity('medium',c).value).toBe(50); expect(scoreSeverity('low',c).value).toBe(30); expect(scoreSeverity('informational',c).value).toBe(10); });
  it('API +10', () => { expect(scoreSeverity('medium', baseContext({isInApiRoute:true})).value).toBe(60); });
  it('PII +10', () => { expect(scoreSeverity('medium', baseContext({handlesPII:true})).value).toBe(60); });
  it('auth +5', () => { expect(scoreSeverity('medium', baseContext({isInAuthCode:true})).value).toBe(55); });
  it('trust +5', () => { expect(scoreSeverity('medium', baseContext({trustBoundaryCount:3})).value).toBe(55); });
  it('low trust no increase', () => { expect(scoreSeverity('medium', baseContext({trustBoundaryCount:1})).value).toBe(50); });
  it('test -15', () => { expect(scoreSeverity('medium', baseContext({isInTestCode:true})).value).toBe(35); });
  it('third-party -10', () => { expect(scoreSeverity('medium', baseContext({isInThirdParty:true})).value).toBe(40); });
  it('stacks', () => { expect(scoreSeverity('high', baseContext({isInApiRoute:true,handlesPII:true,isInAuthCode:true,trustBoundaryCount:2})).value).toBe(100); });
  it('clamp 0', () => { expect(scoreSeverity('informational', baseContext({isInTestCode:true,isInThirdParty:true})).value).toBe(0); });
  it('clamp 100', () => { expect(scoreSeverity('critical', baseContext({isInApiRoute:true,handlesPII:true,isInAuthCode:true,trustBoundaryCount:5})).value).toBe(100); });
  it('levels', () => { const c = baseContext(); expect(scoreSeverity('critical',c).level).toBe('critical'); expect(scoreSeverity('high',c).level).toBe('high'); expect(scoreSeverity('medium',c).level).toBe('medium'); expect(scoreSeverity('low',c).level).toBe('low'); expect(scoreSeverity('informational',c).level).toBe('informational'); });
});
